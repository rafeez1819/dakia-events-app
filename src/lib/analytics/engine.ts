import { CHANNELS, CITIES, EVENT_VALUE_AED, FUNNEL_STEPS } from "./config";
import type {
  AnalyticsEvent,
  Anomaly,
  DailyRollup,
...
function str(params: AnalyticsEvent["params"], key: string, fallback = ""): string {
  const v = params[key];
  return v == null ? fallback : String(v);
}

function asDevice(v: string): "mobile" | "desktop" | "tablet" {
...
function sessionsFrom(events: AnalyticsEvent[]): Map<string, Session> {
...
function scoreSession(s: Session): number {
  let score = 0;
  const n = (name: string) => (s.events.has(name) ? 1 : 0);
  ...
  if (["Dubai", "Abu Dhabi", "Ajman", "Sharjah"].includes(s.city)) score += 4;
  return Math.max(0, Math.min(100, score));
}

function bandOf(score: number): LeadRecord["band"] {
...
function emptyDaily(date: string): DailyRollup {
...
function rollupDays(events: AnalyticsEvent[], rangeDays: number, end: Date): DailyRollup[] {
  ...
  for (const s of sessions.values()) {
    ...
    if (s.city && s.city !== "Unknown") {
      row.geos[s.city] = (row.geos[s.city] ?? 0) + 1;
    }
    if (isLead(s)) {
      row.leads += 1;
      row.conversions += 1;
...
function funnelOf(sessions: Map<string, Session>): FunnelStep[] {
...
function realtimeFrom(events: AnalyticsEvent[]): RealtimeVisitor[] {
  const cutoff = Date.now() - 30 * 60 * 1000;
  const sessions = sessionsFrom(events.filter((e) => e.timestamp >= cutoff));
...
export function buildSnapshotFromEvents(opts: {
  ...
  const prevDaily = rollupDays(prevEvents, rangeDays, new Date(end.getTime() - rangeDays * 86400000));
  const sessions = sessionsFrom(events);
  ...
  const conversionValue = leadSessions.reduce(
    (n, s) => n + (s.value || EVENT_VALUE_AED.lead_generated || 0),
    0,
  );

  const channelUsers: Record<string, Set<string>> = {};
  const channelLeads: Record<string, number> = {};
  const deviceUsers = { mobile: 0, desktop: 0, tablet: 0 };
  const deviceLeads = { mobile: 0, desktop: 0, tablet: 0 };
  const geoUsers: Record<string, Set<string>> = {};
  const geoLeads: Record<string, number> = {};

  for (const s of sessions.values()) {
    channelUsers[s.channel] ??= new Set();
    channelUsers[s.channel]!.add(s.client);
    deviceUsers[s.device] += 1;
    if (s.city && s.city !== "Unknown") {
      geoUsers[s.city] ??= new Set();
  ...
  const deviceMix = (["mobile", "desktop", "tablet"] as const).map((name) => ({
    name,
    users: deviceUsers[name],
    leads: deviceLeads[name],
  }));

  const geoMix = Object.keys({ ...geoUsers, ...Object.fromEntries(CITIES.map((c) => [c, 1])) })
    .map((name) => ({
      name,
  ...
  const pageStats = new Map<string, { views: number; conversions: number; engaged: number }>();
  for (const e of events) {
    if (e.event !== "page_view") continue;
    const group = str(e.params, "content_group", "home");
    const path = group === "home" ? "/" : `/#${group}`;
    const row = pageStats.get(path) ?? { views: 0, conversions: 0, engaged: 0 };
    row.views += 1;
  ...
  const leadsList: LeadRecord[] = leadSessions
    .sort((a, b) => b.ts - a.ts)
    .slice(0, 80)
    .map((s) => {
      const score = scoreSession(s);
      const product = (PRODUCT_SAFE.has(s.product) ? s.product : "UNKNOWN") as LeadRecord["productType"];
      return {
        id: s.id,
        ts: s.ts,
  ...
  const delta = (curr: number, prev: number) => (prev ? (curr - prev) / prev : 0);
...
const PRODUCT_SAFE = new Set(["LED_VIDEO_WALL", "SOUND_SYSTEM", "LIGHTING", "STAGE_RIGGING", "EVENT_PRODUCTION", "AV_RENTAL"]);
